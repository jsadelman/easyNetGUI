...
FullScreenSvgDialog_old::FullScreenSvgDialog_old(QWidget *parent)
...
void FullScreenSvgDialog_old::loadByteArray(QByteArray byteArray)
{
    svg->load(byteArray);
}

void FullScreenSvgDialog_old::clearSvg()
{
...
PlotViewer_old::PlotViewer_old(QString easyNetHomei, QWidget* parent)
      ...
      plotClones()
{
    ...
    fullScreenSvgDialog = new FullScreenSvgDialog_old(this);
    ...
    connect(plotDescriptionUpdater, &ObjectUpdater::objectUpdated, [=](QDomDocument* domDoc, QString plotName)
    {
...
PlotViewer_old::~PlotViewer_old()
{

}

QSvgWidget *PlotViewer_old::currentSvgWidget()
{
    return qobject_cast<QSvgWidget*>(plotPanel->currentWidget());
}

void PlotViewer_old::createToolBars()
{
...
void PlotViewer_old::dispatch_Impl(QSharedPointer<QDomDocument> info)
{
...
void PlotViewer_old::preDispatch(QSharedPointer<QDomDocument> info)
{
...
void PlotViewer_old::showInfo(QSvgWidget *svg)
{
...
void PlotViewer_old::createActions()
{
...
void PlotViewer_old::loadByteArray(QString name, QByteArray byteArray)
{
    if (fullScreen)
...
void PlotViewer_old::setInfoVisible(bool visible)
{
    infoVisible = visible;
    if (infoVisible)
...
void PlotViewer_old::refreshInfo()
{
    if (infoVisible)
        showInfo(currentSvgWidget());
}

void PlotViewer_old::newRPlot(QString name)
{
...
void PlotViewer_old::updateActivePlots()
{
...
QString PlotViewer_old::cloneRPlot(QString name, QString newName)
{
...
void PlotViewer_old::updateAllActivePlots()
{
    QMapIterator<QSvgWidget*, bool> plotIsActiveIt(svgIsActive);
    while (plotIsActiveIt.hasNext()) {
        plotIsActiveIt.next();
...
void PlotViewer_old::setTabState(int index, int state)
{
    QIcon icon;
    switch(state)
    {
...
void PlotViewer_old::setCurrentPlot(QString name)
{
    plotPanel->setCurrentWidget(plotSvg.value(name));
}


void PlotViewer_old::paintEvent(QPaintEvent * event)
{
    if(pend)
    {
        pend=false;
...
void PlotViewer_old::resizeEvent(QResizeEvent*)
{
    resizeTimer->stop();
    resizeTimer->start(250);
}
...
void PlotViewer_old::setSvgActive(bool isActive, QSvgWidget *svg)
{
...
QSvgWidget *PlotViewer_old::newSvg(QString name)
{
...
void PlotViewer_old::resizeTimeout()
{
   resizeTimer->stop();
...
void PlotViewer_old::freeze(QSvgWidget* svg)
{
    if (!svg)
...
void PlotViewer_old::renamePlot()
{
...
void PlotViewer_old::renamePlot(QString oldName, QString newName)
{
...
void PlotViewer_old::deletePlot(QString name)
{
...
void PlotViewer_old::triggerPlotUpdate(QString name)
{
...
void PlotViewer_old::setupFullScreen()
{
    fullScreen = true;
...
void PlotViewer_old::generatePrettyName(QString plotName, QString type, QString subtype, QDomDocument *domDoc)
{
    Q_UNUSED(type)
    Q_UNUSED(subtype)
    Q_UNUSED(domDoc)
...
void PlotViewer_old::addSourceDataframes(QStringList newDataframes)
{
...
void PlotViewer_old::dfSourceModified(QString df)
{
...
QString PlotViewer_old::uniqueName(QString name)
{
    if (!plotSvg.contains(name))
        return name;
    int progNum = 1;
...
QString PlotViewer_old::plotType(QString name)
{
...
void PlotViewer_old::snapshot(QString name)
{
    QSvgWidget* svg;
    if (name.isEmpty())
    {
...
void PlotViewer_old::updateActionEnabledState(QSvgWidget* svg)
{
...
void PlotViewer_old::currentTabChanged(int index)
{
    QSvgWidget* svg = static_cast<QSvgWidget*>(plotPanel->widget(index));
    emit setPlotSettings(plotSvg.key(svg));
    updateActivePlots();
    updateActionEnabledState(svg);
    if (infoVisible)
        showInfo(svg);
