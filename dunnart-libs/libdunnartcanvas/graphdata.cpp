...
//        qWarning("dangling connector!");
    ...
    conn_vec.push_back(conn); // MG: this line should be here, not at the beginning
...
//    qDebug() << "Edge " << addEdge(a,b,idealLength) << a << b;
...
//    qDebug() << "setupMultiEdges";
    ...
    for(list<Connector*>::iterator i=conList.begin();i!=conList.end();i++) {
        ...
        // MG: otherwise a multi edge never becomes single again
        (*i)->m_multiedge_size=1;
        (*i)->m_multiedge_index=0;
        (*i)->m_multiedge_reverse=false;
    }
...
//        qDebug("Duplicates: ");
        ...
        for(list<Connector*>::iterator j=i->begin();j!=i->end();j++) {
            ...
            if (ctr==0) {firstShapeId = p.first->internalId();}
//            qDebug("+-(%d, %d)", p.first->internalId(), p.second->internalId());
            (*j)->m_multiedge_size=i->size();
            (*j)->m_multiedge_index=ctr++;
            (*j)->m_multiedge_reverse=!(firstShapeId == p.first->internalId());
//            qDebug() << (*j)->m_multiedge_size << (*j)->m_multiedge_index;
        }
    }
}
