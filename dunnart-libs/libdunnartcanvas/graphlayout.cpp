...
GraphLayout::GraphLayout(Canvas *canvas) 
    : mode(ORGANIC),
      ...
      ignoreEdges(false) // MG: always false, so that layout is always computed when requested
{
...
{
//    qDebug() << "setInterruptFromDunnart";
...
//    qDebug() << "processReturnPositions: retPositions.size() = " <<
    ...
    m_canvas->m_animation_group->start(/*QAbstractAnimation::DeletionPolicy::DeleteWhenStopped*/);
...
void GraphLayout::clearFixedList()
{
    for_each(fixedPositions.begin(),fixedPositions.end(),delete_object());
    fixedPositions.clear();
    fixedShapeLookup.clear();
...
//    PreIteration preIter(*this);
    cola::PreIteration preIter(locks, resizes); // does nothing
