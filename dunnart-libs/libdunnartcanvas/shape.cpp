...
QPainterPath &ShapeObj::getLoopBasePath()
{
    initSelfLoopPath();
    return loopBasePath;
}
...
QSet<ShapeObj *> ShapeObj::neighbours()
{
    ...
    {
        if ((*conn)->getAttachedShapes().first)
            result.insert((*conn)->getAttachedShapes().first);
        if ((*conn)->getAttachedShapes().second)
...
void ShapeObj::initSelfLoopPath()
{
    // right side of the rectangle referred to item's position
    selfLoopDockingSide = QLineF(shapeRect().topRight() - pos(), shapeRect().bottomRight() - pos());
    dockingLineProportion = 0.4;
    loopDiameterProportion = 0.4;
    loopRotation = -180; // clockwise


    // compute loopBasePath, which will be shifted
...
//                          rectProportion*halfsize,halfsize);
        ...
        loopRect.translate((selfLoopDockingSide.dx() - sgn(selfLoopDockingSide.dx()) * rectProportion*shortSide )/2,
                           (selfLoopDockingSide.dy() - sgn(selfLoopDockingSide.dy()) * shortSide)/2);
...
//                          halfsize,rectProportion*halfsize);
        ...
        loopRect.translate((selfLoopDockingSide.dx() - sgn(selfLoopDockingSide.dx()) * shortSide )/2,
                           (selfLoopDockingSide.dy() - sgn(selfLoopDockingSide.dy()) * rectProportion*shortSide)/2);
    ...
    loopBasePath.arcTo(loopRect,180 + angleOnRect,loopRotation);

    // see libdunnartcanvas Connector::applyMultiEdgeOffset
    double nx = selfLoopDockingSide.dx();
    double ny = selfLoopDockingSide.dy();
    ...
    multi_edge_offset_nx = nx;
    multi_edge_offset_ny = ny;
...
{
    // MG: shape size won't be smaller than its label
