...
QRectF CanvasItem::boundingRectNoExpand() const
{
    // same as boundingRect but without expansion
    assert(!m_painter_path.boundingRect().isNull());
    return m_painter_path.boundingRect();
}
    ...
    if (pass == PASS_SHAPES)
    {
        ...
        else if (type == x_separation)
        {
            newObj = new Separation(node, dunnartURI);
                ...
                ! oldConnTypes.contains(type) && !easyNetTypes.contains(type))
        {
            if (type == x_svgNode)
            {
                shape = new SvgShape();
...
void CanvasItem::moveByDifference(QPointF diff)
{
    // this is the code from mouseMoveEvent
    Actions& actions = canvas()->getActions();
//    QPointF diff = event->pos() - event->lastPos();
