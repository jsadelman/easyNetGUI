    ...
    ShapeObj *shape, uint pinClassID)
{
    ...
    if (canvas())
    {
        canvas()->layout()->initialise(); // MG: to recompute multiedges
...
void Connector::setRoutingCheckPoints(const QList<QPointF>& checkpoints, Avoid::Router *router)
{
    ...
    // these 3 added lines make sure there's no seg fault,
    // yet checkpoints are ignored
    if (router == 0)
        Avoid::Router* router = new Avoid::Router(1); //m_routing_type?
    avoidRef = new Avoid::ConnRef(router);
...
void Connector::disconnect_from(ShapeObj *shape, uint pinClassID)
{
    ...
    setNewLibavoidEndpoint(type); // MG: otherwise dangling connections do not get updated
}
...
void Connector::applySimpleRoute(void)
{
    if (m_src_pt.shape == m_dst_pt.shape)
        qDebug() << "Connector::applySimpleRoute" << "SELF";

    QVariant srcPaddingVar = QVariant();
    QVariant dstPaddingVar = QVariant();
    double connGroupPadding = 0.0;
# if 0
    if (m_src_pt.shape && m_src_pt.shape == m_dst_pt.shape) // selfloop
    {
        QPointF srcPt = m_src_pt.shape->loopSrc();
        m_src_pt.x = srcPt.x();
        m_src_pt.y = srcPt.y();
        QPointF dstPt = m_dst_pt.shape->loopSrc();
        m_dst_pt.x = dstPt.x();
        m_dst_pt.y = dstPt.y();
...
void Connector::applySelfLoop()
{
    if (!m_src_pt.shape)
        return;

    QPainterPath painter_path(mapFromItem(m_src_pt.shape, m_src_pt.shape->getLoopBasePath()));
    if (painter_path.isEmpty())
        return;

    Point offset = multiEdgeOffset(m_src_pt.shape->multiEdgeOffset_nx(), m_src_pt.shape->multiEdgeOffset_ny());
    painter_path.translate(QPointF(offset.x, offset.y));
    prepareGeometryChange();
    setPainterPath(painter_path);
    buildArrowHeadPath(); // works for selfloops too
    update();
}
    ...
    if (m_is_directed)
    {
        if (m_dst_pt.shape)
        {
            ...
            if (path_size >= 2)
            {
                ...
                // MG: this magically works also if path is an arch, so for self loops too.
...
Avoid::Point Connector::multiEdgeOffset(double nx, double ny)
{
    if (!m_src_pt.shape && !m_dst_pt.shape)
        return Point(0,0);

    double pos = (double)m_multiedge_index - ((double)m_multiedge_size-1.)/2.;
    double reverse = m_multiedge_reverse ? -1 : 1;
    double pointOffset = 12.;
#if 1

    double _largeNumber = pointOffset * (double)m_multiedge_size;
    if (abs(nx/ny) >1) // steeper than 45 degrees (x and y are inverted)
        pointOffset = qMin(pointOffset,
                           qMin(m_src_pt.shape ? m_src_pt.shape->width() : _largeNumber,
                                m_dst_pt.shape ? m_dst_pt.shape->width() : _largeNumber) / (((double)m_multiedge_size-1.)*2.));
    else
        pointOffset = qMin(pointOffset,
                           qMin(m_src_pt.shape ? m_src_pt.shape->height() : _largeNumber,
                                m_dst_pt.shape ? m_dst_pt.shape->height() : _largeNumber) / (((double)m_multiedge_size-1.)*2.));

#endif

    return Point(nx*pos*pointOffset*reverse, ny*pos*pointOffset*reverse);
...
void Connector::applyMultiEdgeOffset(Point& p1, Point& p2, bool justSecond)
{
    ...
    Point offset = multiEdgeOffset(nx, ny);
...
//    qDebug() << "Connector::applyMultiEdgeOffset" << p1.id << p2.id << justSecond;
//    qDebug() << p1.x << p1.y << p2.x << p2.y <<m_multiedge_size << m_multiedge_index ;
}
